#include "dispetchermodel.h"

DispetcherModel::DispetcherModel()
{
...
///        Get proc info
/// ---------------------------------------------------------------
QMap<QString,QStringList> DispetcherModel::get_proc_info()
{
    process_info.clear();
    ...
    while(ent = readdir(proc))
    {
        ...
        QMap<QString,QString> info=DispetcherModel::get_process_info(vm, rss,pid);
...
/// ---------------------------------------------------------------
QString DispetcherModel::get_running(char* pid)
{
    char tbuf[32];
    char *cp;
    int fd;
...
///        Get status info
/// ---------------------------------------------------------------
QMap<QString, QString> DispetcherModel::status_info()
{
    struct sysinfo info;
...
///        Kill process
/// ---------------------------------------------------------------
void DispetcherModel::kill_process(QString PID)
{
    errno = 0;
    pid_t pid=PID.toInt();
...
/// ---------------------------------------------------------------
std::vector<size_t> DispetcherModel::get_cpu_times() {
...
bool DispetcherModel::get_cpu_times(size_t &idle_time, size_t &total_time) {
...
///        Process info
/// ---------------------------------------------------------------
QMap<QString,QString> DispetcherModel::get_process_info(double& vm_usage, double& resident_set,long proc_id)
{
   using std::ios_base;
   using std::ifstream;
   using std::string;
